(ns postharvest.render-html
  ...
  ## Styling has no external dependency

  The page is styled from the DADS primitives this repo already vendors
  into `docs/index.html`, read at build time (see `style-css`). There is
  deliberately no `jp-go-digital-design-system` git dependency: it would
  make regeneration network-dependent, and its `tokens/bridge-css`
  bridges `--hig-*` names rather than the `--color-*` primitives this
  page uses.
...
;; ── stylesheet ──
;;
;; The console is styled from the DADS (デジタル庁デザインシステム)
;; primitives THIS REPO ALREADY SHIPS: `scripts/vendor.cljs` vendored the
;; upstream token block into `docs/index.html` (the product face), so the
;; console reads its token values out of that file at build time.
;;
;; Deliberately NOT a `jp-go-digital-design-system` git dependency:
;;   - a git dep makes this build network-dependent, and the console is
;;     regenerated by offline fleet CI nodes;
;;   - the pinned `tokens/bridge-css` bridges `--hig-*` names, which is a
;;     DIFFERENT vocabulary from the `--color-*` primitives these consoles
;;     use, so wiring it in silently UNSTYLES the page.
;; Reading the repo's own vendored copy also means the console cannot
;; drift away from the product face: both render the same hex values.

(def ^:private product-page-path
  "The product face, whose `<style>` carries the vendored DADS `:root`
  token block. Also the reason the console needs no CSS of its own beyond
  the small skin below."
  "docs/index.html")

(defn- parse-root-tokens
  "Parse `:root { --name: value; ... }` out of vendored CSS into a map of
  token name -> value. Returns nil when no `:root` block is present, so
  the caller can fail loudly instead of emitting a page whose every colour
  silently resolves to nothing.

  Splits on `;` rather than on newlines: the vendored file is
  prettier-formatted, so a long declaration wraps across lines
  (`--font-family-sans` is the one that actually does). A line-based
  parser drops exactly those tokens and reports them as undefined."
  [css]
  (let [s (str/index-of css ":root {")
        e (when s (str/index-of css "\n}" s))]
    (when e
      (into {}
            (for [decl (str/split (subs css (+ s 7) e) #";")
                  :let [[_ k v] (re-matches #"(?s)\s*(--[a-z0-9-]+)\s*:\s*(.+?)\s*" decl)]
                  :when k]
              ;; collapse the wrap so the emitted :root block is one line
              ;; and byte-stable regardless of upstream formatting
              [k (str/replace (str/trim v) #"\s+" " ")])))))

(def ^:private skin-rules
  "The whole visual vocabulary this page uses. Every colour, face and rule
  is a `var(--…)` into the vendored DADS primitives --- no raw hex is
  written here, which is what keeps the console and the product page the
  same design system rather than two lookalikes.

  Selectors are plain elements plus the small class vocabulary the
  renderer emits (`.card` `.bar` `.badge` `.ok` `.warn` `.critical`
  `.muted`); this document contains no `.dads-*` components, so the rules
  need no guards against them."
  [["body" (str "font-family:var(--font-family-sans);"
                "color:var(--color-neutral-solid-gray-800);"
                "background:var(--color-neutral-white);line-height:1.8;"
                "max-width:96rem;margin:0 auto;padding:2rem 1rem 4rem")]
   ["h1" (str "font-size:2.125rem;font-weight:700;line-height:1.4;margin:0;"
              "color:var(--color-neutral-solid-gray-900)")]
   ["h2" (str "font-size:1.5rem;font-weight:700;line-height:1.5;"
              "margin:0 0 .75rem;padding-bottom:.5rem;"
              "border-bottom:1px solid var(--color-neutral-solid-gray-200);"
              "color:var(--color-neutral-solid-gray-900)")]
   ["h3" (str "font-size:1.25rem;font-weight:700;line-height:1.5;"
              "margin:0 0 .5rem;color:var(--color-neutral-solid-gray-900)")]
   ["p" "margin:0 0 1rem"]
   ["ul" "margin:0 0 1rem;padding-left:1.25rem"]
   ["li" "margin-block:.25rem"]
   ["table" (str "display:block;overflow-x:auto;max-width:100%;"
                 "border-collapse:collapse;width:100%;"
                 "margin:.75rem 0 .5rem;font-size:.875rem")]
   ["th,td" (str "border:1px solid var(--color-neutral-solid-gray-300);"
                 "padding:.5rem .75rem;text-align:left;vertical-align:top")]
   ["th" (str "background:var(--color-neutral-solid-gray-50);font-weight:700;"
              "white-space:nowrap;color:var(--color-neutral-solid-gray-900)")]
   ["code" (str "font-family:var(--font-family-mono);"
   ...
   ;; Governor dispositions. Weight carries the same signal as colour, so
   ...
   ;; A HARD hold is the one thing on this page a human may not override,
   ;; so its ledger row is tinted. The tint is the PRIMITIVE red-50 step:
   ;; `--color-semantic-error-2` is NOT a weak counterpart of `-1` (both
   ;; are dark: red-800 / red-900) and using it here would produce dark
   ;; text on a dark ground.
   ["tr[data-hard-hold] td" (str "background:var(--color-primitive-red-50);"
                                 "border-color:var(--color-primitive-red-200)")]
   [".card" (str "border:1px solid var(--color-neutral-solid-gray-200);"
                 "border-radius:12px;padding:1.25rem 1.5rem;margin:1.25rem 0;"
                 "background:var(--color-neutral-white)")]
   [".badge" (str "display:inline-block;font-size:.8125rem;font-weight:700;"
   ...
   [".bar" (str "display:flex;align-items:center;gap:.75rem;"
                "padding:.75rem 0;margin-bottom:1rem;"
                "border-bottom:1px solid var(--color-neutral-solid-gray-200)")]
   ["footer" (str "margin-top:3rem;padding-top:1.5rem;"
                  "border-top:1px solid var(--color-neutral-solid-gray-200);"
                  "color:var(--color-neutral-solid-gray-600);font-size:.875rem")]])

(def ^:private skin-css
  (str/join "\n" (map (fn [[sel decls]] (str sel "{" decls "}")) skin-rules)))

(defn- css-var-names
  "Every `--token` referenced through `var(…)` in a chunk of CSS."
  [css]
  (set (map second (re-seq #"var\(\s*(--[a-z0-9-]+)" css))))

(defn- token-closure
  "The subset of `tokens` reachable from `seeds`, following `var(…)`
  references inside token VALUES. DADS semantic tokens are aliases --
  `--color-key-900: var(--color-primitive-blue-900)` -- so emitting only
  the directly-referenced names would produce a page whose colours
  resolve to nothing. Returns names only; the caller emits values."
  [tokens seeds]
  (loop [seen #{} todo (set seeds)]
    (if-let [n (first todo)]
      (let [todo' (disj todo n)]
        (if (or (seen n) (not (contains? tokens n)))
          (recur (conj seen n) todo')
          (recur (conj seen n)
                 (into todo' (css-var-names (get tokens n))))))
      (into (sorted-set) (filter tokens seen)))))

(defn- style-css
  "`:root` block carrying ONLY the DADS primitives this page actually
  references (transitively closed over aliases), followed by the skin.
  Throws rather than emitting an unstyled page if the product face has
  lost its vendored token block or a referenced primitive."
  []
  (let [f (io/file product-page-path)
        _ (when-not (.exists f)
            (throw (ex-info (str "cannot read " product-page-path
                                 " -- run this from the repository root"
                                 " (clojure -M:dev:render-html)")
                            {:path (.getAbsolutePath f)})))
        css (slurp f)
        tokens (or (parse-root-tokens css)
                   (throw (ex-info (str "no vendored DADS :root token block in "
                                        product-page-path
                                        " -- cannot style the console")
                                   {:path product-page-path})))
        needed (token-closure tokens (css-var-names skin-css))
        missing (sort (remove tokens (css-var-names skin-css)))]
    (when (seq missing)
      (throw (ex-info "skin references DADS primitives the product page does not define"
                      {:missing missing :path product-page-path})))
    (str "/* DADS primitives extracted at build time from " product-page-path
         " (vendored from digital-go-jp/design-system-example-components-html,\n"
         "   MIT, (c) 2025 デジタル庁). Only the "
         (count needed) " tokens this page references are inlined. */\n"
         ":root{"
         (str/join "" (map #(str % ":" (get tokens %) ";") needed))
         "}\n"
         skin-css "\n")))
...
(defn render
  ...
  (let [ledger (vec (store/audit-trail st))
    ...
    (str
     ...
     (section "バッチ登録簿"
              "施設が抱えるロット。「最終状態」列は台帳の最後のファクトから導出している。"
     ...
     "（時刻・乱数をページに出さない）。配色・字面はこの repo が "
     "<code>docs/index.html</code> に vendoring 済みの"
     "デジタル庁デザインシステム(DADS)の primitive をビルド時に読み出したもので、"
     "外部依存を持たない（生成はオフラインで完結する）。</p>\n"
     "</footer>\n"
     "</body></html>\n")))
...
(defn -main [& args]
  (let [out (or (first args) "docs/samples/operator-console.html")
    ...
    ;; CSS var closure: a `var(--x)` that nothing defines does not fail
    ;; loudly in a browser, it silently renders as no colour at all. Check
    ;; the FINISHED document (not just the skin) so a token referenced from
    ;; anywhere in the page has to be defined in the page.
    (let [declared (set (map second (re-seq #"(--[a-z0-9-]+)\s*:" html)))
          dangling (sort (remove declared (css-var-names html)))]
      (when (seq dangling)
        (throw (ex-info "rendered console references CSS variables it never defines"
                        {:dangling dangling}))))
    (io/make-parents out)
    (spit out html)
    (println "wrote" out
