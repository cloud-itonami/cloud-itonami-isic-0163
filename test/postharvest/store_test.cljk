...
(deftest production-batch-test
  (testing "retrieve an existing batch"
    (let [batch-data {:crop-lot-type :tea/black-leaf :moisture-percent 3.0}
...
(deftest log-batch-test
  (testing "logging a batch marks it as processed"
    (let [st {:batches {}}
          batch-data {:crop-lot-type :tea/black-leaf}
  ...
  (testing "logging preserves batch data"
    (let [st {:batches {}}
          batch-data {:crop-lot-type :tea/black-leaf :moisture-percent 3.0}
          result (store/log-batch st "batch-001" batch-data)]
      (is (= (:crop-lot-type (get-in result [:batches "batch-001"])) :tea/black-leaf))
      (is (= (:moisture-percent (get-in result [:batches "batch-001"])) 3.0)))))

;; ──────────────────────── Shipment Finalization ──────────────────────

(deftest finalize-shipment-test
  (testing "finalizing a batch marks it as finalized"
    (let [st {:batches {"batch-001" {:crop-lot-type :tea/black-leaf}}}
